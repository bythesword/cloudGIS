...
float getwaves_RAYMARCH(vec2 position, int iterations) {
...
float getwaves_NORMAL(vec2 position, int iterations) {
  float iter = 0.0; // this will help generating well distributed wave directions
  float frequency = 1.0; // frequency of the wave, this will change every iteration
  float timeMultiplier = 2.0; // time multiplier for the wave, this will change every iteration
  float weight = 1.0;// weight in final sum for the wave, this will change every iteration
...
float raymarchwater(vec3 camera, vec3 start, vec3 end, float depth) {
  ...
  for(int i=0; i < 64; i++) {
    ...
    float height = getwaves_RAYMARCH(pos.xz, ITERATIONS_RAYMARCH) * depth - depth;
...
vec3 normal(vec2 pos, float e, float depth) {
  vec2 ex = vec2(e, 0);
  float H = getwaves_NORMAL(pos.xy, ITERATIONS_NORMAL) * depth;
  vec3 a = vec3(pos.x, H, pos.y);
  return normalize(
    cross(
      a - vec3(pos.x - e, getwaves_NORMAL(pos.xy - ex.xy, ITERATIONS_NORMAL) * depth, pos.y), 
      a - vec3(pos.x, getwaves_NORMAL(pos.xy + ex.yx, ITERATIONS_NORMAL) * depth, pos.y + e)
    )
  );
}
...
vec3 getRay(vec2 fragCoord) {
  ...
  //这里返回的每个点的视角
  vec3 proj = normalize(vec3(uv.x, uv.y-2., 1.5));
  if(iResolution.x < 600.0) {
    return proj;
  }
  return createRotationMatrixAxisAngle(vec3(0.0, -1.0, 0.0), 3.0 * (( 0.5) * 2.0 - 1.0)) 
    * createRotationMatrixAxisAngle(vec3(1.0, 0.0, 0.0), 0.5 + 1.5 * (( 1.5) * 2.0 - 1.0))
    * proj;
}
...
void main() {
  // get the ray
  vec2 fragCoord=v_uv;
  // vec2 fragCoord=vec2(v_uv.x,v_uv.y*10.);
